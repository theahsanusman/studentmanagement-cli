...
class Student extends Person {
    rollNumber: number;
    courses: string[];
    constructor(id: string, name: string, age: number, rollNumber: number, courses: string[]) {
        super(id, name, age);
...
class Instructor extends Person {
    salary: number;
    courses: string[];
    constructor(id: string, name: string, age: number, salary: number, courses: string[]) {
        super(id, name, age);
        this.salary = salary;
        this.courses = courses;
    }
...
class Course {
    ...
    students: string[];
    instructor: string;
    constructor(id: string, name: string, students: string[], instructor: string) {
        this.id = id;
        this.name = name;
...
class Department {
    ...
    name: string;
    courses: string[];
    constructor(id: string, name: string, courses: string[]) {
        this.id = id;
        this.name = name;
        this.courses = courses;
    }
...
const instructors: { [id: string]: Instructor } = {};
const courses: { [id: string]: Course } = {};
const departments: { [id: string]: Department } = {};
...
async function addNewCourse() {
    ...
    if (!course.name) {
        ...
        if (course.instructorId && Object.keys(instructors).findIndex(i => i === course.instructorId) >= 0) instructorFound = true;
...
async function enrollStudent():Promise<any> {
    ...
    if (!answer.studentId || !answer.courseId) {
        ...
        if (answer.studentId && Object.keys(students).findIndex(i => i === answer.studentId) >= 0) studentFound = true;
        if (!studentFound) {
            console.log('Invalid Student Id');
            return enrollStudent();
        }
        if (answer.courseId && Object.keys(courses).findIndex(i => i === answer.courseId) >= 0) courseFound = true;
        if (!courseFound) {
...
async function assignCourse():Promise<any> {
    const answer = await inquirer.prompt([
        {
            type: 'input',
            name: 'instructorId',
    ...
    if (!answer.instructorId || !answer.courseId) {
        ...
        let instructorFound = false;
        let courseFound = false;
        if (instructorId && Object.keys(instructors).findIndex(i => i === instructorId) >= 0) instructorFound = true;
        if (!instructorFound) {
            console.log('Invalid Instructor Id');
            return assignCourse();
        }
        if (courseId && Object.keys(courses).findIndex(i => i === courseId) >= 0) courseFound = true;
        if (!courseFound) {
            console.log('Invalid Course Id');
            return assignCourse();
        }
...
async function addCourseIntoDepartment():Promise<any> {
    const answer = await inquirer.prompt([
        {
            type: 'input',
            name: 'departmentId',
    ...
    if (!answer.departmentId || !answer.courseId) {
        ...
        let courseFound = false;
        if (departmentId && Object.keys(departments).findIndex(i => i === departmentId) >= 0) departmentFound = true;
        if (!departmentFound) {
